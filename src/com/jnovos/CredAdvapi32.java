...
/**
 * This class exposes functions from credential manager on Windows platform via
 * JNA.
 *
 ...
 * @param <E>
 */
interface CredAdvapi32<E> extends StdCallLibrary {
	CredAdvapi32<?> INSTANCE = (CredAdvapi32<?>) Native.loadLibrary("Advapi32", CredAdvapi32.class,
	 ...
	 *
	 * typedef struct _CREDENTIAL_ATTRIBUTE { LPTSTR Keyword; DWORD Flags; DWORD
	 * ValueSize; LPBYTE Value; } CREDENTIAL_ATTRIBUTE, *PCREDENTIAL_ATTRIBUTE;
	 *
	 */
	class CREDENTIAL_ATTRIBUTE extends Structure {
		 ...
		 * <CompanyName>_<Name>. This member cannot be longer than
		...
		/**
		 * Data associated with the attribute. By convention, if Value is a text string,
		 * then Value should not include the trailing zero character and should be in
		 * UNICODE.
		 *
		 * Credentials are expected to be portable. The application should take care to
		 * ensure that the data in value is portable. It is the responsibility of the
		 * application to define the byte-endian and alignment of the data in Value.
		 */
	 ...
	 *
	 * typedef struct _CREDENTIAL { DWORD Flags; DWORD Type; LPTSTR TargetName;
	 * LPTSTR Comment; FILETIME LastWritten; DWORD CredentialBlobSize; LPBYTE
	 * CredentialBlob; DWORD Persist; DWORD AttributeCount; PCREDENTIAL_ATTRIBUTE
	...
	class CREDENTIAL extends Structure {
		 ...
		 * bits should be initialized as zero and not otherwise altered to permit future
		 * enhancement.
		 *
		...
		/**
		 * The name of the credential. The TargetName and Type members uniquely identify
		 * the credential. This member cannot be changed after the credential is
		 * created. Instead, the credential with the old name should be deleted and the
		 ...
		 * larger than CRED_MAX_CREDENTIAL_BLOB_SIZE (512) bytes.
		 */
		public int CredentialBlobSize;

		/**
		 * Secret data for the credential. The CredentialBlob member can be both read
		 * and written. If the Type member is CRED_TYPE_DOMAIN_PASSWORD, this member
		 * contains the plaintext Unicode password for UserName. The CredentialBlob and
		 * CredentialBlobSize members do not include a trailing zero character. Also,
		 * for CRED_TYPE_DOMAIN_PASSWORD, this member can only be read by the
		 * authentication packages.
		 *
		 * If the Type member is CRED_TYPE_DOMAIN_CERTIFICATE, this member contains the
		 * clear test Unicode PIN for UserName. The CredentialBlob and
		 * CredentialBlobSize members do not include a trailing zero character. Also,
		 * this member can only be read by the authentication packages.
		 *
		 * If the Type member is CRED_TYPE_GENERIC, this member is defined by the
		 * application. Credentials are expected to be portable. Applications should
		 * ensure that the data in CredentialBlob is portable. The application defines
		 * the byte-endian and alignment of the data in CredentialBlob.
		 */
		 ...
		 * credential. This member can be read and written. Its value cannot be greater
		 * than CRED_MAX_ATTRIBUTES (64).
		 */
		public int AttributeCount;

		/**
		 * Application-defined attributes that are associated with the credential. This
		 * member can be read and written.
		 */
		// TODO: Need to make this into array
		public CREDENTIAL_ATTRIBUTE.ByReference Attributes;

		/**
		 * Alias for the TargetName member. This member can be read and written. It
		 * cannot be longer than CRED_MAX_STRING_LENGTH (256) characters.
		 *
		 * If the credential Type is CRED_TYPE_GENERIC, this member can be non-NULL, but
		 * the credential manager ignores the member.
		 */
		public String TargetAlias;

		/**
		 * The user name of the account used to connect to TargetName. If the credential
		 * Type is CRED_TYPE_DOMAIN_PASSWORD, this member can be either a
		 * DomainName\UserName or a UPN.
		 *
		 * If the credential Type is CRED_TYPE_DOMAIN_CERTIFICATE, this member must be a
		 * marshaled certificate reference created by calling CredMarshalCredential with
		 * a CertCredential.
		 *
		 * If the credential Type is CRED_TYPE_GENERIC, this member can be non-NULL, but
		 * the credential manager ignores the member.
	 ...
	 *                    CRED_TYPE_* defined types.
	 * @param flags       Currently reserved and must be zero.
	 * @param pcredential Out - Pointer to a single allocated block buffer to return
	 *                    the credential. Any pointers contained within the buffer
	 *                    are pointers to locations within this single allocated
	 *                    block. The single returned buffer must be freed by calling
	 *                    <code>CredFree</code>.
	 *
	 ...
	 * @param flags      Flags that control the function's operation. The following
	 *                   flag is defined. CRED_PRESERVE_CREDENTIAL_BLOB: The
	 *                   credential BLOB from an existing credential is preserved
	 *                   with the same credential name and credential type. The
	 *                   CredentialBlobSize of the passed in Credential structure
	 *                   must be zero.
	 *
	 ...
	 *                   CRED_TYPE_* defined types. For a list of the defined types,
	 *                   see the Type member of the CREDENTIAL structure. If the
	 *                   value of this parameter is CRED_TYPE_DOMAIN_EXTENDED, this
	 *                   function can delete a credential that specifies a user name
	 *                   when there are multiple credentials for the same target.
	 *                   The value of the TargetName parameter must specify the user
	 *                   name as Target|UserName.
	...
	/**
	 * //TODO I create this method beacause it not exist
	 * 
	 * The CredEnumerate function enumerates the credentials from the user's
	 * credential set. The credential set used is the one associated with the logon
	 ...
	 *
	 * @param filter       Pointer to a null-terminated string that contains the
	 *                     filter for the returned credentials. Only credentials
	 *                     with a TargetName matching the filter will be returned.
	 *                     The filter specifies a name prefix followed by an
	 *                     asterisk. For instance, the filter "FRED*" will return
	 *                     all credentials with a TargetName beginning with the
	 *                     string "FRED".
	 * @param flag         Reserved and must be zero.
	 * @param count        Count of the credentials returned in the Credentials
	 *                     array.
	 * @param pCredentials Pointer to an array of pointers to credentials. The
	 *                     returned credential is a single allocated block. Any
	 *                     pointers contained within the buffer are pointers to
	 *                     locations within this single allocated block. The single
	 *                     returned buffer must be freed by calling CredFree.
	 *
	 * @return True if CredEnumerate succeeded, false otherwise
	 *
	 * @throws LastErrorException GetLastError
	 */

	boolean CredEnumerateW(String filter, int flag, IntByReference count, PointerByReference pCredentials)
